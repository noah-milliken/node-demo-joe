...
CCRouter.route("/") // set router to base of route from server.js
  ...
  .post(async (req, res) => {
    if (!req.body.company || !req.body.applicationDate) {
      return res
        .status(400)
        .json({ error: "company and application date are required" });
    }
  ...
  .put(async (req, res) => {
    ...
    const updatedApplication = await Application.update(req.body.application, {
      where: { id: req.body.application.id },
    });
...
CCRouter.route("/:id").delete(async (req, res) => {
  if (!req?.params?.id) {
    return res.status(400).json({ error: "id is required" });
  }
